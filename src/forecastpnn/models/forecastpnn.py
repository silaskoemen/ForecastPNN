...
class ForecastPNNDayMultiFeature(nn.Module):
    def __init__(self, past_units=30, n_features=3, n_layers=3, hidden_units=[24, 12, 4]):
        super().__init__()
        self.past_units = past_units
        self.n_features = n_features
        
        # Modified LSTM layers to accept n_features as input_size
        self.lstm1 = nn.LSTM(input_size=(past_units, n_features), hidden_size=hidden_units[0], 
                            num_layers=n_layers, batch_first=True)
        self.lstm2 = nn.LSTM(input_size=hidden_units[0], hidden_size=hidden_units[1], 
                            num_layers=n_layers, batch_first=True)
        ...
        # Rest of the layers
        self.bnorm2 = nn.BatchNorm1d(num_features=hidden_units[1])
        self.bnorm3 = nn.BatchNorm1d(num_features=hidden_units[2])
    ...
    def forward(self, x):
        ...
        dist = Normal(df = self.past_units-1, loc=loc, scale=scale)
        return torch.distributions.Independent(dist, reinterpreted_batch_ndims=1)
