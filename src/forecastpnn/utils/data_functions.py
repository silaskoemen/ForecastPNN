...
def get_dataset_days(dataset: pd.DataFrame, reference_col: str, past_units: int = 12, return_df: bool = False, time_format: str = "%Y-%m-%d", dow: bool = True) -> torch.utils.data.Dataset:
    ...
    if dow:
        logger.warning("Day of the weeks cannot be used as feature, so keyword dow will be ignored.")
        dow = False
    ...
    dataset = dataset.reindex(pd.date_range(start=dataset.index.min(), end=dataset.index.max(), freq='D')).fillna(0).astype(int)
    if return_df:
        return dataset
    else:
        return ReportingDataset(dataset, past_units=past_units, dow=dow)

def get_dataset_weeks(dataset: pd.DataFrame, reference_col: str, past_units: int = 12, return_df: bool = False, time_format: str = "%Y-%m-%d", dow: bool = True) -> torch.utils.data.Dataset:
    """ Function to transform a given dataframe to a torch dataset.
...
def get_dataset_days_to_weeks(dataset: pd.DataFrame, reference_col: str, past_units: int = 12, return_df: bool = False, time_format: str = "%Y-%m-%d", dow: bool = True) -> torch.utils.data.Dataset:
    """ Function to transform a given dataframe to a torch dataset.
...
def get_dataset(dataset: pd.DataFrame, reference_col: str,  weeks_in: bool = False, weeks_out: bool = True, past_units: int = 12, return_df: bool = False, time_format: str = "%Y-%m-%d", dow: bool = True) -> torch.utils.data.Dataset:
    ...
    if weeks_in:
        assert not weeks_out, "If incoming data is indexed by weeks, output data cannot be returned by day."
    ...
    if weeks_in:
        ...
        if weeks_out:
            ...
            # Fill missing weeks with 0
            dataset = dataset.reindex(epi_weeks_date_range(min_week=dataset.index.min(), max_week=dataset.index.max())).fillna(0)
